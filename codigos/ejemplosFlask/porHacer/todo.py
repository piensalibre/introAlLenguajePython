...
from porHacer.auth import login_required
...
def index():
    db, c = get_db()
    c.execute(
        'select t.id, t.description, u.username, t.completed, t.created_at from todo t JOIN user u on t.created_by = u.id and created_by = %s order by created_at desc',
        (g.user['id'],)
    )
    todos = c.fetchall()
